...
class LoanService:
    ...
    def submit_application(
        ...
        if not customer:
            raise NotFoundError(
                f"Customer with ID {application_data.customer_id} not found"
            )
        
        if not customer.is_active:
            raise BusinessRuleViolationError(
                "Cannot submit loan application for inactive customer"
            )
        
        # Check for existing pending applications
        existing_pending = self.db.query(LoanApplication).filter(
    ...
    def get_application(self, application_id: UUID) -> LoanApplication:
        ...
        if not application:
            raise NotFoundError(
                f"Loan application with ID {application_id} not found"
            )
    ...
    def review_application(
        ...
        Raises:
            NotFoundError: If application not found
            BusinessRuleViolationError: If application cannot be reviewed
        """
        ...
        if review_data.status == 'APPROVED':
            ...
            application.approved_amount = review_data.approved_amount or application.requested_amount
    ...
    def disburse_loan(
        self,
        application_id: UUID,
        disbursement_data: LoanDisbursementRequest
    ) -> LoanApplication:
        """
        ...
        Raises:
            NotFoundError: If application not found
            BusinessRuleViolationError: If loan cannot be disbursed
        """
        application = self.get_application(application_id)
